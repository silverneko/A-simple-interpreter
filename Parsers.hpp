...
class Parser{
  public:
    ...
    Parser operator [] (std::function<void (const std::string&)>);
...
Parser Parser::operator [] (function<void (const std::string&)> f){
...
Parser say(const std::string& mess, std::function<void (const std::string&)> f){
  return Parser([mess, f](Parser::Iterator _, Parser::Iterator __){
    f(mess);
    return goodParse(_);
  });
}
